...
{
    // Dosyadan yüklenecek orijinal sütunlar
    ...
    [LoadColumn(30)] public bool Label { get; set; }

    // Türetilmiş özellikler artık getter ve setter'a sahip
    ...
    // Pipeline'da kullanacağınız isimle uyumlu olması için DayFeature ve HourFeature kullanıyoruz
    [ColumnName("DayFeature")] public float DayFeature { get; set; }

    [ColumnName("HourFeature")] public float HourFeature { get; set; }

    [ColumnName("SampleWeight")] public float SampleWeight { get; set; } = 1.0f;

    // Örneğin, ağırlığı ayarlamak için yardımcı metod
