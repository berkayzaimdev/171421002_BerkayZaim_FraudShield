...
const TransactionManagement: React.FC = () => {
  ...
  const handleLoadTransactionDetail = async (transaction: Transaction) => {
    try {
      ...
      // Önce mevcut transaction'ı set et
      setCurrentTransaction(transaction);
      ...
      // Hata durumunda da detay modalını aç ama boş veri ile
      setDetailData({
                ...
                <Box>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 2 }}>
                    <Box sx={{
                      ...
                      <Typography variant="body2" sx={{ color: 'white', fontSize: '1rem' }}>💰</Typography>
                    </Box>
                    ...
                    <CardContent>
                      ...
                      <Stack spacing={2}>
                        <Typography variant="body2" color="textSecondary">
                          Her context'i ayrı ayrı analiz edebilir veya tümünü birden çalıştırabilirsiniz.
                        </Typography>

                        {/* Tekil Context Butonları */}
                        <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
                          {analysisSteps.map((step) => (
                            <Button
                              ...
                              size="small"
                              startIcon={step.icon}
                              onClick={() => runAnalysisStep(analysisSteps.findIndex(s => s.id === step.id), currentTransaction!)}
                              disabled={step.status === 'running' || isAnalyzing}
                              sx={{
                                borderColor: step.color,
                                color: step.color,
                                borderWidth: step.status === 'completed' ? 2 : 1,
                                bgcolor: step.status === 'completed' ? `${step.color}10` : 'transparent',
                                '&:hover': {
                                  borderColor: step.color,
                        ...
                        {/* Tüm Analizleri Çalıştır Butonu */}
                        <Button
                          variant="contained"
                          startIcon={<PlayIcon />}
                          onClick={() => currentTransaction && runAnalysisStep(0, currentTransaction)}
                      ...
                      {isAnalyzing && (
                        <Box sx={{ mt: 2 }}>
                          <Typography variant="subtitle2" gutterBottom sx={{
                            ...
                            📊 Analiz İlerlemesi ({currentStepIndex + 1}/{analysisSteps.length})
                          </Typography>
                                  ...
                                  icon={
                                    <Avatar
                                      sx={{
                                        ...
                                        color: 'white',
                                        border: step.status === 'running' ? '2px solid #2196f3' : 'none'
                                      }}
                                    >
                                      {step.status === 'running' ? (
                                  ...
                                  <Box>
                                    <Typography variant="body2" fontWeight="medium" sx={{
                                      color: step.status === 'failed' ? 'error.main' :
                                        step.status === 'completed' ? 'success.main' : 'text.primary'
                                    }}>
                                      {step.name}
                                    </Typography>
                                    <Typography variant="caption" color="text.secondary">
                                ...
                                <StepContent>
                                  {step.result && (
                                    <Card variant="outlined" sx={{
                                      mt: 1,
                                      border: step.status === 'completed' ? '2px solid #4caf50' : '2px solid #f44336',
                                      bgcolor: step.status === 'completed' ? '#f1f8e9' : '#ffebee'
                                    }}>
                                      <CardContent sx={{ p: 2 }}>
                                        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
                                          ...
                                          <Chip
                                            label={step.result.decision || step.result.resultType || 'Unknown'}
                                            size="small"
                                            color={step.result.isSuccess ? 'success' : 'error'}
                                            sx={{ fontSize: '0.7rem' }}
                                          />
                                        </Box>
                                        <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 1 }}>
                                          <Box>
                                            ...
                                            <Typography variant="body2" sx={{ fontWeight: 'bold' }}>
                                              {getRiskScoreValue(step.result.riskScore).toFixed(1)}%
                                            </Typography>
                                          </Box>
                                          <Box>
                                            <Typography variant="caption" color="text.secondary">Kurallar:</Typography>
                                            <Typography variant="body2" sx={{ fontWeight: 'bold' }}>
                                              {step.result.triggeredRuleCount || 0}
                                            </Typography>
                                          </Box>
                                        </Box>

                                        {/* Fraud Probability */}
                                        {step.result.fraudProbability && (
                                          <Box sx={{ mt: 1 }}>
                                            <Typography variant="caption" color="text.secondary">Dolandırıcılık Olasılığı:</Typography>
                                            <Typography variant="body2" sx={{ fontWeight: 'bold', color: 'error.main' }}>
                                              {(step.result.fraudProbability * 100).toFixed(1)}%
                                            </Typography>
                                          </Box>
                                        )}

                                        {/* Anomaly Score */}
                                        {step.result.anomalyScore && (
                                          <Box sx={{ mt: 1 }}>
                                            <Typography variant="caption" color="text.secondary">Anomali:</Typography>
                                            <Typography variant="body2" sx={{ fontWeight: 'bold', color: 'warning.main' }}>
                                              {step.result.anomalyScore.toFixed(2)}
                                            </Typography>
                                          </Box>
                                        )}

                                        {/* Applied Actions */}
                                        {step.result.appliedActions && step.result.appliedActions.length > 0 && (
                                          <Box sx={{ mt: 1 }}>
                                            <Typography variant="caption" color="primary.main" sx={{ fontWeight: 'bold' }}>
                                              🎯 {step.result.appliedActions.length} aksiyon
                                            </Typography>
                                          </Box>
                                        )}
                      ...
                      {analysisResults && Object.keys(analysisResults).length > 0 && (
                        <Stack spacing={3}>
                          ...
                          <Card variant="outlined">
                            <CardContent>
                              <Typography variant="subtitle1" gutterBottom>Genel Risk Durumu</Typography>

                              {/* Ana analiz sonucu varsa (initial-analysis) */}
                              {analysisResults['initial-analysis'] && (
                                <Box sx={{ mb: 2 }}>
                                  <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
                                    <Typography variant="body2">Dolandırıcılık Olasılığı</Typography>
                                    <Typography variant="h6" sx={{ color: getRiskColor(getRiskLevelValue(analysisResults['initial-analysis'].riskLevel)) }}>
                                      {(analysisResults['initial-analysis'].fraudProbability * 100)?.toFixed(1) || '0'}%
                                    </Typography>
                                  </Box>
                                  <LinearProgress
                                    variant="determinate"
                                    value={(analysisResults['initial-analysis'].fraudProbability * 100) || 0}
                                    sx={{
                                      height: 12,
                                      borderRadius: 6,
                                      '& .MuiLinearProgress-bar': {
                                        bgcolor: getRiskColor(getRiskLevelValue(analysisResults['initial-analysis'].riskLevel))
                                      }
                                    }}
                                  />
                                  ...
                                  <Box sx={{ display: 'flex', justifyContent: 'space-between', mt: 2 }}>
                                    <Chip
                                      label={getRiskLevelValue(analysisResults['initial-analysis'].riskLevel)}
                                      sx={{ bgcolor: getRiskColor(getRiskLevelValue(analysisResults['initial-analysis'].riskLevel)), color: 'white' }}
                                    />
                                    <Chip
                                      label={analysisResults['initial-analysis'].decision === 'Approve' ? 'Onaylandı' :
                                        analysisResults['initial-analysis'].decision === 'Deny' ? 'Reddedildi' :
                                          analysisResults['initial-analysis'].decision || 'Bilinmiyor'}
                                      color={
                                        analysisResults['initial-analysis'].decision === 'Approve' || analysisResults['initial-analysis'].decision === 'Approved' ? 'success' :
                                          analysisResults['initial-analysis'].decision === 'Deny' || analysisResults['initial-analysis'].decision === 'Blocked' ? 'error' : 'warning'
                                      }
                                    />
                                  </Box>
                                  ...
                                  <Box sx={{ mt: 2, display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 1 }}>
                                    <Box>
                                      <Typography variant="caption">Anomali Skoru</Typography>
                                      <Typography variant="body2">{analysisResults['initial-analysis'].anomalyScore?.toFixed(2) || '0.00'}</Typography>
                                    </Box>
                                    <Box>
                                      <Typography variant="caption">Durum</Typography>
                                      <Typography variant="body2">{analysisResults['initial-analysis'].status === 'Completed' ? 'Tamamlandı' : 'Bilinmiyor'}</Typography>
                                    </Box>
                                    <Box>
                                      <Typography variant="caption">Tetiklenen Kurallar</Typography>
                                      <Typography variant="body2">{analysisResults['initial-analysis'].triggeredRuleCount || 0}</Typography>
                                    </Box>
                                    <Box>
                                      <Typography variant="caption">İşlem Süresi</Typography>
                                      <Typography variant="body2">{analysisResults['initial-analysis'].processingTime || 0}ms</Typography>
                                    </Box>
                                  </Box>

                                  {/* Applied Actions */}
                                  {analysisResults['initial-analysis']?.appliedActions && analysisResults['initial-analysis'].appliedActions.length > 0 && (
                                    <Box>
                                      <Typography variant="subtitle1" gutterBottom>🎯 Uygulanan Aksiyonlar</Typography>
                                      <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
                                        {analysisResults['initial-analysis'].appliedActions.map((action: string, index: number) => (
                                  ...
                                  {analysisResults['initial-analysis']?.riskFactors && analysisResults['initial-analysis'].riskFactors.length > 0 && (
                                    <Box sx={{ mt: 2 }}>
                                      <Typography variant="subtitle1" gutterBottom>
                                        ⚠️ Risk Faktörleri ({analysisResults['initial-analysis'].riskFactors.length})
                                      </Typography>
                                      <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
                                        {analysisResults['initial-analysis'].riskFactors.map((factor: any, index: number) => (
                                          <Chip
                                            key={index}
                                            label={`${factor.code}: ${factor.description}`}
                                            size="small"
                                            color={factor.severity === 'Critical' ? 'error' :
                                              factor.severity === 'High' ? 'warning' : 'default'}
                                            variant="outlined"
                                            sx={{ maxWidth: '200px' }}
                                          />
                                        ))}
                            ...
                            <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 2 }}>
                              <Card variant="outlined" sx={{ p: 2, bgcolor: '#f5f5f5' }}>
                                ...
                                <Typography variant="h5" sx={{
                                  ...
                                  {analysisResults['initial-analysis'] ?
                                    ((analysisResults['initial-analysis'].fraudProbability * 100) || 0).toFixed(1) : '0.0'}%
                                </Typography>
                              </Card>
                              ...
                              <Card variant="outlined" sx={{ p: 2, bgcolor: '#f5f5f5' }}>
                                ...
                                <Typography variant="h5" sx={{
                                  ...
                                  {analysisResults['initial-analysis'] ?
                                    (analysisResults['initial-analysis'].anomalyScore || 0).toFixed(2) : '0.00'}
                                </Typography>
                              </Card>

                              <Card variant="outlined" sx={{ p: 2, bgcolor: '#f5f5f5' }}>
                                <Typography variant="caption" color="text.secondary" sx={{ fontWeight: 'bold' }}>
                                  ⚠️ Tetiklenen Kurallar
                                </Typography>
                                <Typography variant="h5" sx={{
                                  fontWeight: 'bold',
                                  color: 'info.main',
                                  ...
                                  {analysisResults['initial-analysis'] ?
                                    (analysisResults['initial-analysis'].triggeredRuleCount || 0) : 0}
                                </Typography>
                              </Card>
                              ...
                              <Card variant="outlined" sx={{ p: 2, bgcolor: '#f5f5f5' }}>
                                ...
                                <Typography variant="h5" sx={{
                                  ...
                                  {analysisResults['initial-analysis'] ?
                                    (analysisResults['initial-analysis'].processingTime || 0) : 0}ms
                                </Typography>
                              </Card>
                            </Box>
                          </Box>
                          {/* Context Analiz Sonuçları */}
                          {Object.entries(analysisResults).filter(([key]) => key !== 'initial-analysis').length > 0 && (
                            <Box>
                              <Typography variant="subtitle1" gutterBottom sx={{
                                display: 'flex',
                                alignItems: 'center',
                              ...
                              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2 }}>
                                {Object.entries(analysisResults).filter(([key]) => key !== 'initial-analysis').map(([contextKey, contextResult]: [string, any]) => (
                                  <Card key={contextKey} variant="outlined" sx={{ minWidth: 280, flex: '1 1 280px', bgcolor: '#f9f9f9' }}>
                                    <CardContent>
                                      <Typography variant="subtitle2" sx={{ fontWeight: 'bold', mb: 1 }}>
                                        {analysisSteps.find(s => s.id === contextKey)?.name || contextKey}
                                      </Typography>
                                      ...
                                      <Box sx={{ mb: 1 }}>
                                        ...
                                        <Typography variant="body2" sx={{ fontWeight: 'bold' }}>
                                          {contextResult.decision === 'Approved' ? 'Onaylandı' :
                                            contextResult.decision === 'Deny' ? 'Reddedildi' :
                                              contextResult.decision === 'PermanentlyBlocked' ? 'Kalıcı Engellendi' :
                                                contextResult.decision || 'Bilinmiyor'}
                                        </Typography>
                                      </Box>
                                      {contextResult.triggeredRules && contextResult.triggeredRules.length > 0 && (
                                        <Box sx={{ mb: 1 }}>
                                          <Typography variant="caption" color="warning.main" sx={{ fontWeight: 'bold' }}>
                                            Tetiklenen Kural:
                                          </Typography>
                                          <Typography variant="body2">
                                            {contextResult.triggeredRules[0].ruleName || contextResult.triggeredRules[0].ruleCode || contextResult.triggeredRules[0].ruleId}
                                          </Typography>
                                        </Box>
                                      )}
                                      {contextResult.appliedActions && contextResult.appliedActions.length > 0 && (
                                        <Box sx={{ mb: 1 }}>
                                          <Typography variant="caption" color="primary.main" sx={{ fontWeight: 'bold' }}>
                                            Aksiyonlar:
                                          </Typography>
                                          <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5, mt: 0.5 }}>
                                            {contextResult.appliedActions.map((action: string, idx: number) => (
                                              <Chip key={idx} label={action} size="small" color="primary" variant="outlined" />
                                            ))}
                                          </Box>
                                        </Box>
                                      )}
                                      {contextResult.resultMessage && (
                                        <Box sx={{ mb: 1 }}>
                                      ...
                                      <Typography variant="caption" color="text.secondary" sx={{ display: 'block' }}>
                                        İşlem Süresi: {contextResult.processingTime && contextResult.processingTime > 0 ?
                                          `${contextResult.processingTime}ms` :
                                          (contextResult._measuredTime ? `${contextResult._measuredTime}ms` : 'Ölçülmedi')}
                                      </Typography>
                                    </CardContent>
                                  </Card>
                                ))}
                          ...
                          {/* Detaylı Risk Faktörleri Listesi */}
                          {(() => {
                            // Tüm analiz sonuçlarından risk faktörlerini topla
                            ...
                            Object.entries(analysisResults).forEach(([key, result]) => {
                              if (result.riskFactors && Array.isArray(result.riskFactors)) {
                                result.riskFactors.forEach((factor: any) => {
                                  allRiskFactors.push({
                                    ...factor,
                                    source: key, // Hangi analizden geldiğini belirt
                                    contextName: analysisSteps.find(s => s.id === key)?.name || key
                                  });
                                });
                              }
                            });

                            return allRiskFactors.length > 0 ? (
                              <Box>
                                <Typography variant="subtitle1" gutterBottom sx={{
                                  display: 'flex',
                                  alignItems: 'center',
                                  ...
                                  ⚠️ Detaylı Risk Faktörleri Listesi ({allRiskFactors.length})
                                </Typography>

                                <Card variant="outlined">
                                  <CardContent>
                                    <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2 }}>
                                      {allRiskFactors.map((factor: any, index: number) => (
                                        <Card key={index} variant="outlined" sx={{
                                          ...
                                          <CardContent sx={{ p: 2 }}>
                                            ...
                                            <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 1, mb: 1 }}>
                                              ...
                                              <Box>
                                                <Typography variant="caption" color="text.secondary">Kaynak:</Typography>
                                                <Typography variant="body2" sx={{ fontWeight: 'bold' }}>
                                                  {factor.source === 'Rule' ? 'Kural' : factor.source}
                                                </Typography>
                                              </Box>
                                            </Box>
                                            ...
                                            {factor.detectedAt && (
                                              <Box>
                                                <Typography variant="caption" color="text.secondary">Tespit Zamanı:</Typography>
        ...
        <Box sx={{
          ...
          {currentTransaction && (
            <>
                        ...
                        {detailData.transaction && (
                          <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 2 }}>
                            ...
                            <Box>
                              ...
                              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                <Typography variant="body2" sx={{ fontSize: '1.2rem' }}>
                                  {getTypeIcon(detailData.transaction.type || currentTransaction.type)}
                                </Typography>
                                <Typography variant="body2">
                                  {getTypeText(detailData.transaction.type || currentTransaction.type)}
                                </Typography>
                              </Box>
                            </Box>
                            <Box>
                      ...
                      <CardContent>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 2 }}>
                          <Box sx={{
                            ...
                            <Typography variant="body2" sx={{ color: 'white', fontSize: '1rem' }}>⚠️</Typography>
                          </Box>
                          <Typography variant="h6" sx={{ fontWeight: 'bold' }}>
                            Risk Faktörleri ({detailData.riskFactors?.length || 0})
                          </Typography>
                        </Box>
              ...
              {/* Footer */}
              <Box sx={{ p: 3, bgcolor: 'grey.100', borderTop: 1, borderColor: 'divider' }}>
                <Button variant="contained" onClick={() => setShowDetailModal(false)} fullWidth>
                  Kapat
                </Button>
              </Box>
            </>
